...
void close_dev(int fd) { close(fd); }
...
int main(int argc, char *argv[]) {
  ...
  if (pid == 0) {
    /* read process */
    for (;;) {
      /* read data from /dev/ttyS0, and write the data into buffer_rec,wait for
         the kernel to get the data.
      */
      flag = 0;
      rec_count = 0;
      buffer_ptr = buffer;
...
#ifdef _DEBUG
          printf("\n Service recieve :\n");
          for (i = 0; i < rec_count; i++) printf(" %02x", rec_ptr[i] & 0xff);
          printf("\n");
#endif
    ...
    for (;;) {
      ...
      */
      ...
      if ((nbytes = read(fd_tx, tx_ptr, BUFFER_SIZE - 1)) > 0) {
        ...
        printf("\nSend bytes:%d\n", nbytes);
        printf("\nServer send\n");
        for (i = 0; i < tx_count; i++) printf(" %02x", buffer_tx_pack[i]);
        printf("\n");
#endif
        tx_length = 0;
